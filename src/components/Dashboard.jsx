...
const Dashboard = ({ onLogout }) => {
  ...
  // Función test para generar reporte PDF
  const generarReportePDF = () => {
    const elemento = document.getElementById("tablitaLista");
    ...
    html2canvas(elemento).then((canvas) => {
        ...
        const posY = (pageHeight - imgHeight) / 4;
        
        pdf.addImage(imgData, "PNG", posX, posY, imgWidth, imgHeight);
        pdf.save("reporte_polizas.pdf");
    });
  };
  ...
  const isBotonReporte = location.pathname === "/dashboard";
  // Cerrar el modal
  const closeModal = () => {
  ...
  return (
    <div className="container-fluid d-flex vh-100" style={{ width: "100%" }}>
        ...
        <ul>
          ...
          <li className="dropdown">
            ...
            <div className="dropdown-content">
                ...
                <Link to="polizas/crear">Crear Póliza</Link>
              
              
                <Link to="polizas/buscar">Buscar Póliza</Link>
          ...
          <li className="dropdown">
            ...
            <div className="dropdown-content">
                ...
                <Link to="usuarios/lista">Listar Usuarios</Link>
          ...
          <li className="dropdown2">
            <button className="btn-donate" onClick={handleLogout}>
              Cerrar Sesión
            </button>
          </li>
        </ul>
      ...
      {isBotonReporte && (<div id="botonReporte">
            <button className="btn-donate" onClick={generarReportePDF}>
                Generar Reporte
            </button>
        </div>)}
      {isDashboard && (
          ...
          <tbody>
            {polizas.map((poliza) => (
              <tr key={poliza.id_poliza}>
                ...
                <td className="centradoTable">
                  {formatoFecha(poliza.fecha_inicio)}
                </td>
                <td className="centradoTable">
                  {formatoFecha(poliza.fecha_vencimiento)}
                </td>
                    ...
                    <div className="card__left" style={{ width: "40%;" }}>
                      <div className="item">
                        <input
                          type="text"
                          name="tipo_seguro"
                          value={polizaSeleccionada.tipo_seguro}
                      ...
                      <div className="item">
                        <input
                          type="date"
                          name="fecha_inicio"
                          value={polizaSeleccionada.fecha_inicio}
                          onChange={handleChange}
                          required
                        />
                      </div>
                      <div className="item">
                        <input
                          ...
                          value={polizaSeleccionada.fecha_vencimiento}
                          onChange={handleChange}
                          required
                        />
                      </div>
                      <div className="item">
                        <input
                          ...
                          value={polizaSeleccionada.monto_asegurado}
                          onChange={handleChange}
                          required
                        />
                      </div>
                      <div className="item">
                        <input
                          ...
                          value={polizaSeleccionada.detalles_adicionales}
                          onChange={handleChange}
                          required
                        />
                      </div>
                      <div className="item">
                        <input
                          type="number"
                          name="id_usuario"
                          value={polizaSeleccionada.id_usuario}
                          onChange={handleChange}
                          required
                        />
                      </div>
